...
from numpy import histogram,sqrt,exp,linspace,mean,random,array,floor,ptp,std,shape
...
def Fetchfile(rootloc,keyword="",cropnum=2,skipfirst=True,verbose=0):
    ...
    for dirname, dirnames, filenames in os.walk(rootloc):
        for filename in filenames:
            if filename.startswith('input') or filename.startswith('output'): #skips over guff
                continue
            if filename[-3:]=="txt":
    ...
    for i,afile in enumerate(set(RunNames)):
        ...
        if skipfirst and afile.find("00000")>=0: #ignores transitional files
        ...
        fs = {int(fn[1]):fullloc for fn,fullloc in zip(fs,fls)} #grouped files
...
def CombineFiles(rootloc,splitby='_0',verbose=0):
    ...
    SameName = set([val.split(splitby)[0] for val in UniqueNames]) #all unique files
    ...
    for rootname in SameName:
        ...
        for j in range(1,6)+[7,8]:
            for i,filename in enumerate(fFiles):
                ...
                df = pds.read_csv(filename[j],skiprows=4,sep=";",index_col=0)
            ...
            if not os.path.exists(newloc):
                os.makedirs(newloc)
            fdf.to_csv(newloc+"/F"+str(j)+"Run_"+rootname[4:]+".txt",sep=";",header="Time;Ampl")
...
def WhenWasTheFileCreated(rootloc,verbose=0):
    ...
    TimeData = [{"uniquename":un,"mtime":os.path.getmtime(fn[3]),"ctime":os.path.getctime(fn[3])} for un,fn in zip(*filedata)]
...
def normfit(xdata,ydata,yerr=None,ScaleGuess=0.05,PeakGuess = None,failedfitmax=5,verbose=0):
    '''
    ...
    while True:
        ...
        try:
            ...
            if type(err) is not float: #success!
                break
        except RuntimeError:
            return (None,None),None
        ...
        try:
            ...
            if verbose>0:
                ...
                assert 1==2
                print("outside range!")
            x0 = [ydata[array(xdata).searchsorted(random.uniform(0.4,0.7))],ScaleGuess*random.uniform(0.5,5),1]
        
        FailedToFitCounter += 1
    
    
    p1,p2,p3 = param
    chival = chisquaretest(normdist(xdata,p1,p2,p3),ydata,sqrt(ydata),len(xdata)+len(param)-1)
    if verbose>0:
...
def PrintValues(param,err,axis=None,roundto=4,size=10,offset=0):
    ...
    for i,(p,e) in enumerate(zip(param[:-1],err[:-1])):
        CurrentString = ":  " + str(round(p,roundto))+PorM+str(round(e,roundto))
        if axis is not None:
            axis.text(0.6, 0.9-(i+2*offset)*0.08,ParamNames[i], horizontalalignment='left',verticalalignment='center',transform=axis.transAxes,size=size)
            axis.text(0.7, 0.9-(i+2*offset)*0.08,CurrentString, horizontalalignment='left',verticalalignment='center',transform=axis.transAxes,size=size)
        else:
...
def FindPhotoPeakEvents(File,binrange=(0.1,1),fitrange=(0.3,0.9),Bins=200,leftsigma=3,rightsigma=5,MinSamples=100,axis=None,SkipRows=4,verbose=0):
        '''
        ...
        Condition = (edges > edgemin) & (edges < edgemax) #Peak is probably in this range
        
        if verbose>0:
            print("Number of data points to fit to is",len(edges[Condition]))
    
        (param,err),chival = normfit(edges[Condition],freq[Condition],yerr=sqrt(freq[Condition]),ScaleGuess=0.05,verbose=verbose)
...
def LocatePhotoPeaks(afile,binrange=(0.1,1),factor=8,MinValue=100,Step=0.05,leftsigma=2,rightsigma=2,
    ...
    for x,y in MaxPeaks:
        ...
        try:
            param,err = curve_fit(normdist,xdata,ydata,p0=[guessloc,0.05,1])
        except RuntimeError:
            if verbose>0:
                print("fit failed")
            continue
    ...
    try:
        ...
        photopeakindices = list(df[(df.Ampl > p1-leftsigma*p2) & (df.Ampl < p1+rightsigma*p2)].index)
    except NameError:
    ...
    if secondpeakfound:
        ...
        secondpeakindices = list(df[(df.Ampl > s1-leftsigma*s2) & (df.Ampl < s1+rightsigma*s2)].index)
        ...
        if GenerateImages:
            ...
            axis.plot(X,Y,'-')
            PrintValues(secondpeakparam,secondpeakerrors,axis,offset=1,size=10)
            axis.set_xlim(xmin,xmax)
    else:
    ...
    return (photopeakindices,peakparam,peakerrors),(secondpeakindices,secondpeakparam,secondpeakparam)
...
def DelayPeakFitting(filenames,uniquename,**kwargs):
    ...
    if workingon=="2396": #a bit hacky but it's a one off case comparison
        ...
        try:
            crystallength = int(B[:-1]) #Position B refers to the scintillator under interest always
        except ValueError:
    ...
    LeftFirstPeak,LeftSecondpeak = LocatePhotoPeaks(filenames[1],binrange=LeftPheRange,SkipRows=SkipRows,axis=ax1,verbose=verbose)
    ...
    RightFirstPeak,RightSecondpeak = LocatePhotoPeaks(filenames[2],binrange=RightPheRange,SkipRows=SkipRows,axis=ax2,verbose=verbose)
    ...
    IndicesThree = FindFirstPhePeak(filenames[7],axis=ax3,SkipRows=SkipRows,verbose=verbose)
    IndicesFour = FindFirstPhePeak(filenames[8],axis=ax4,SkipRows=SkipRows,verbose=verbose)
    
    if GenerateImages:
        fig.tight_layout()
        for Ext in Extensions:
            fig.savefig(ImageSaveLocation+'/'+Ext+'/'+SampleNames+'_'+uniquename+'_'+'IndexData'+'.'+Ext)
        show()
        
    
    if SelectIndices == 0:
        Indices = list(set(IndicesOne) & set(IndicesTwo) & set(IndicesThree) & set(IndicesFour))
    elif SelectIndices == 1:
        Indices = list(set(IndicesOne) & set(IndicesTwoSecond) & set(IndicesThree) & set(IndicesFour))
    elif SelectIndices == 2:
        Indices = list(set(IndicesOne) & (set(IndicesTwo) | set(IndicesTwoSecond)) & set(IndicesThree) & set(IndicesFour))
    else:
    ...
    Frequency,Values = histogram(fdf.Ampl,range=timerange,bins=ptp(timerange)/dt)
    ...
    xmin,xmax = timerange
    (param,err),chival = normfit(Values,Frequency,yerr=sqrt(Frequency),ScaleGuess=100,PeakGuess=100,failedfitmax=100,verbose=verbose) #fit to CTR peak
    ...
    if errortype == 'lsq': #error generated by curve_fit()
        ...
        (param,err),chival = EmpiricalBootstrap(fdf.Ampl,p2,filenames[3],GenerateImages=GenerateImages,ImageKey=ImageKey,verbose=verbose)
        locerr,scaleerr,amperr = param
    elif errortype == 'scikits':
            #(fdf,loc=0,sigma=100,leftsigma=2,rightsigma=2,verbose=0)
        locerr,scaleerr,amperr = ScikitsBootstrap(fdf,loc=p1,scale=p2,verbose=verbose)
    ...
    if GenerateImages: 
        ...
        PrintValues(param,[locerr,scaleerr,amperr],ax) #prints errors from selected method
        fig.tight_layout()
        
        for Ext in Extensions:
            fig.savefig(ImageSaveLocation+'/'+Ext+'/'+SampleNames+'_'+uniquename+'_'+'CTR'+'.'+Ext)
        show()
    
    DataDict = {"uniquename":uniquename,"location":p1,"locationerr":locerr,"scale":p2,"scaleerr":scaleerr,"mtime":ModificationTime,"ctime":CreationTime,
    "amplitude":p3,"amplitudeerr":amperr,"chisquared":chival,"numofsamples":len(fdf.Ampl),"length":crystallength,
    "SampleA":A,"SampleB":B,
    ...
    "RSPloc":RightSecondPeakParam[0],"RSPscale":RightSecondPeakParam[1],
    "RSPlocerr":RightSecondPeakError[0],"RSPscaleerr":RightSecondPeakError[1],
    }
    return DataDict
...
def FindDelayData(filenames,uniquename,outputdir,verbose=0):
    ...
    IndicesOne,LeftPhotopeakLoc,FitValLeft = FindPhotoPeakEvents(filenames[1],fitrange=Leftfitregion,verbose=verbose)
    IndicesTwo,RightPhotopeakLoc,FitValRight = FindPhotoPeakEvents(filenames[2],fitrange=Rightfitregion,verbose=verbose)
    
    if (FitValLeft == None) or (FitValRight == None):
        print("Fit Failed")
        return None    
...
def FitToDelayData(DelayValues,timerange=1000,GenerateImages=True,verbose=0):
    '''
    Loads cropped data and fits Gaussian
    Calculates error using bootstrap
    '''
    
    freq,binedges = histogram(DelayValues,bins=2*timerange/25+1,range=(-timerange,timerange))
    binedges = 0.5*(binedges[1:]+binedges[:-1])
    
    binedges = binedges[freq>0]
    freq = freq[freq>0]
    
    (param,err),chival = normfit(binedges,freq,yerr=sqrt(freq),ScaleGuess=100,verbose=verbose) #fit to CTR peak
    p1,p2,p3 = param
...
def ScikitsBootstrap(fdf,loc=0,scale=100,leftsigma=5,rightsigma=5,minsamples=100,verbose=1):
    '''
...
#    fRawData = fdf.Ampl[abs(fdf.Ampl) < 1000]
    fRawData = fdf.Ampl[(fdf.Ampl > loc-leftsigma*scale) & (fdf.Ampl < loc+rightsigma*scale)]
    if verbose>0:
...
def ParametricBootstrap(loc,scale,N,Runs=500,verbose=0):
    ...
    estloc,estscale = zip(*[stats.norm.fit(nsam) for nsam in RandomSamples[::Runs]])
...
def RandomSample(xdata,cdf,NSamples,verbose=0):
    ...
    try:
        Sample = [xdata[cdf.searchsorted(r,side='left')] for r in R if r<limit]
    except IndexError:
        print(R)
        
    return array(Sample)
    
def EmpiricalBootstrap(rawdata,fitscale,filename,NRuns=500,timerange=(-500,500),dt=25,GenerateImages=True,ImageKey="",FetchData=False,verbose=0):
    '''
    ...
    for _ in range(NRuns):
        RndSample = RandomSample(ecdf.x,ecdf.y,NSamples=len(rawdata)) #step 2, generate new sample
        
        Values,Frequency = zip(*stats.itemfreq(RndSample))
        Frequency = array(Frequency,dtype=float)

        (param,err),chival = normfit(Values,Frequency,yerr=sqrt(Frequency),ScaleGuess=100,verbose=verbose) #step 3, fit to CTR peak
    ...
    BinEdges = 0.5*(BinEdges[1:]+BinEdges[:-1])
    (param,err),chival = normfit(BinEdges,Freq,ScaleGuess=100,verbose=verbose) #step 3, fit to CTR peak
    p1,p2,p3 = param
    ...
    if GenerateImages:
        ...
        for Ext in Extensions:
            fig.savefig(ImageSaveLocation+'/'+Ext+'/'+SampleNames+'_'+Key+'_'+ImageKey+'_'+'empiricalbootstrap'+'.'+Ext)
        show()
...
def ProcessFiles(fileloc,**kwargs):   
    ...
    GeneratedData = []
    UniqueNames,Files = Fetchfile(fileloc,skipfirst=skipfirst,verbose=0) #BORING :P
    GeneratedData += [DelayPeakFitting(fs,un,**kwargs) for fs,un in zip(Files,UniqueNames)]
    show()
    
    GeneratedData = [gi for gi in GeneratedData if type(gi) is dict] #gets rid of failed events
...
def FetchDataFrame(rootloc,workingon,Combined=False,ErrorType='scikits',verbose=0):
    '''
    Retrieves dataframe matching conditions
    '''
    if Combined:
        filename = rootloc+'-Combined'+'/'+workingon+'-'+ErrorType+'.csv'
    else:
    ...
    try:
        ...
        if verbose>0:
            print("Check folder for any .csv files. Have you run ProcessFiles yet?")
        return 1
...
def GenerateCTR(df,reference=ufloat(42,2),verbose=0):
    ...
    '''
    TotalSigma = [ufloat(grp.scale,grp.scaleerr) for key,grp in df.groupby('uniquename')]
    
    if isinstance(type(reference),type(ufloat(0,0))): #a reference value is provided
...
def PrintLatexDOI(singlesampledf,RoundLvl = 1,withsamplename=False,verbose=0):
    
    singlesampledf["Time Resolution (ps)"] = [str(round(grp["TimeResolution"],RoundLvl))+"$\pm$"+str(round(grp["TimeResolutionError"],RoundLvl)) for key,grp in singlesampledf.groupby("DOI")]
    singlesampledf["CTR (ps)"] = [str(round(grp["CTR"],RoundLvl-1))+"$\pm$"+str(round(grp["CTRError"],RoundLvl-1)) for key,grp in singlesampledf.groupby("DOI")]
    singlesampledf = singlesampledf.sort("DOI")
    
    if withsamplename:
        print(singlesampledf.to_latex(index=False,columns=["DOI","SampleB","Time Resolution (ps)","CTR (ps)"]))            
    else:
        print(singlesampledf.to_latex(index=False,columns=["DOI","Time Resolution (ps)","CTR (ps)"]))        

        
def MeanCTR(adf):
    CTRvals = [unc.ufloat(a,b) for a,b in zip(adf.CTR,adf.CTRError)]
    return mean(CTRvals)
...
def savefig(name,fig,**kwargs):
    ...
    '''
    ImageSaveLocation = kwargs.get('isl','/home/mbrown/Desktop/Dropbox/TransferReport/images')
