...
from numpy import std,mean,floor,sqrt,random
...
def LoadData(fileloc,verbose=False):
    ...
    defaultparam = {'crystal':'None','grease':1.0,'dB':0.0,'source':1.0,'orientation':'top','outer':"unwrapped"}  
    if verbose>0:
        print(fileloc)
    SkipRows = []
    Keys = []
...
def CalculatePhe(location,locationerr,dB = 15,verbose=0):
    ...
    qch = 160E-15 #C
    G = 1.58E7*(3390/4650) #with gain correction factor due to broken dynode
